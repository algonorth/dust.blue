...
if(musicOn){music.play().then(()=>fadeTo(1,FADE_IN)).catch(()=>{musicOn=false;setSoundUI(false);});}else{fadeTo(0,FADE_OUT);}}
soundBtn.addEventListener('click',toggleSound);const savedSound=(()=>{try{return localStorage.getItem('tby-sound');}catch{return null;}})();if(savedSound==='on'){const resume=(e)=>{if(soundResumed)return;if(soundBtn.contains(e.target))return;toggleSound();};addEventListener('pointerdown',resume);addEventListener('keydown',resume);addEventListener('touchend',resume);}
