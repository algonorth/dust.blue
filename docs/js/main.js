...
musicOn=!musicOn;setSoundUI(musicOn);try{localStorage.setItem('tby-sound',musicOn?'on':'off');}catch{}
if(musicOn){music.play().then(()=>fadeTo(1,FADE_IN)).catch(()=>{musicOn=false;soundResumed=false;setSoundUI(false);});}else{fadeTo(0,FADE_OUT);}}
soundBtn.addEventListener('click',toggleSound);const savedSound=(()=>{try{return localStorage.getItem('tby-sound');}catch{return null;}})();if(savedSound!=='off'){const resume=(e)=>{if(soundResumed)return;if(e.type==='pointerdown'&&e.pointerType!=='mouse')return;if(soundBtn.contains(e.target))return;toggleSound();};addEventListener('pointerdown',resume);addEventListener('keydown',resume);addEventListener('touchend',resume);}
