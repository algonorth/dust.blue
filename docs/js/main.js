...
const viewW=()=>document.documentElement.clientWidth||innerWidth;const IS_TOUCH=matchMedia('(pointer: coarse)').matches;const IS_SMALL=viewW()<720;const DPR=Math.min(devicePixelRatio||1,IS_TOUCH?1.5:1.75);const DUST_COUNT=(IS_TOUCH||IS_SMALL)?48000:130000;renderer.setPixelRatio(DPR);renderer.setSize(viewW(),innerHeight);renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.1;const scene=new THREE.Scene();scene.background=new THREE.Color(0x030308);const camera=new THREE.PerspectiveCamera(46,viewW()/innerHeight,0.1,900);camera.position.set(0,4,66);function track(keys){return(p)=>{if(p<=keys[0][0])return keys[0][1];for(let i=0;i<keys.length-1;i++){const[p0,v0]=keys[i],[p1,v1]=keys[i+1];if(p<=p1){const t=(p-p0)/(p1-p0);const s=t*t*(3-2*t);return v0+(v1-v0)*s;}}
return keys[keys.length-1][1];};}
const clamp=(x,a,b)=>Math.max(a,Math.min(b,x));const lerp=(a,b,t)=>a+(b-a)*t;let frameK=1,portK=0;function applyViewport(){camera.aspect=viewW()/innerHeight;const wide=clamp(Math.pow(1.45/camera.aspect,0.5),1,1.42);camera.fov=46*wide;frameK=clamp(camera.aspect/1.45,0.38,1);portK=clamp(1-camera.aspect/1.45,0,0.7);camera.updateProjectionMatrix();}
...
addEventListener('resize',()=>{applyViewport();renderer.setSize(viewW(),innerHeight);composer.setSize(viewW(),innerHeight);measureVh();readScroll();});const MUSIC_SRC='bgm/abyss_bloom.mp3';const MUSIC_VOL=1.0;const FADE_IN=5;const FADE_OUT=0.8;const soundBtn=document.getElementById('sound');let music=null,musicOn=false,musicGain=0,fadeRaf=0;function fadeTo(target,seconds){cancelAnimationFrame(fadeRaf);const from=musicGain,t0=performance.now();(function tick(now){const k=Math.min((now-t0)/(seconds*1000),1);musicGain=from+(target-from)*k;music.volume=clamp(musicGain*MUSIC_VOL,0,1);if(k<1)fadeRaf=requestAnimationFrame(tick);else if(target===0)music.pause();})(t0);}
...
musicOn=!musicOn;setSoundUI(musicOn);if(musicOn){music.play().then(()=>fadeTo(1,FADE_IN)).catch(()=>{musicOn=false;soundResumed=false;setSoundUI(false);});}else{fadeTo(0,FADE_OUT);}}
soundBtn.addEventListener('click',toggleSound);const resume=(e)=>{if(soundResumed)return;if(e.type==='pointerdown'&&e.pointerType!=='mouse')return;if(soundBtn.contains(e.target))return;toggleSound();};addEventListener('pointerdown',resume);addEventListener('keydown',resume);addEventListener('touchend',resume);document.addEventListener('visibilitychange',()=>{if(!music||!musicOn)return;if(document.hidden)music.pause();else music.play().catch(()=>{});});if(history.scrollRestoration)history.scrollRestoration='manual';scrollTo(0,0);requestAnimationFrame(()=>{document.querySelector('.ch-title').classList.add('visible');});frame();window.__tby={starUni,coronaUni,bloom,corona,star,dustUniforms,renderer,bh,bhUni,get music(){return music;}};