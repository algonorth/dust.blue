...
const clamp=(x,a,b)=>Math.max(a,Math.min(b,x));const lerp=(a,b,t)=>a+(b-a)*t;const dustGeo=new THREE.BufferGeometry();{const cloud=new Float32Array(DUST_COUNT*3);const disk=new Float32Array(DUST_COUNT*3);const shell=new Float32Array(DUST_COUNT*3);const seed=new Float32Array(DUST_COUNT*4);const NCLUMP=26;const clumps=[];for(let i=0;i<NCLUMP;i++){const a=(i/NCLUMP)*Math.PI*2*2.4+Math.random()*0.9;clumps.push({x:Math.cos(a)*(10+Math.random()*34)*(Math.random()>0.5?1:-0.8),y:(Math.random()-0.5)*42,z:(Math.random()-0.5)*46,r:4+Math.random()*11,});}
