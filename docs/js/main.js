...
const clamp=(x,a,b)=>Math.max(a,Math.min(b,x));const lerp=(a,b,t)=>a+(b-a)*t;let frameK=1;function applyViewport(){camera.aspect=innerWidth/innerHeight;const wide=clamp(Math.pow(1.45/camera.aspect,0.5),1,1.42);camera.fov=46*wide;frameK=clamp(camera.aspect/1.45,0.38,1);camera.updateProjectionMatrix();}
