...
addEventListener('resize',()=>{applyViewport();renderer.setSize(viewW(),innerHeight);composer.setSize(viewW(),innerHeight);measureVh();readScroll();});const MUSIC_SRC='bgm/abyss_bloom.mp3';const MUSIC_VOL=1.0;const FADE_IN=5;const FADE_OUT=0.8;const soundBtn=document.getElementById('sound');let music=null,musicOn=false,musicGain=0,fadeRaf=0;function fadeTo(target,seconds){cancelAnimationFrame(fadeRaf);const from=musicGain,t0=performance.now();(function tick(now){const k=Math.min((now-t0)/(seconds*1000),1);musicGain=from+(target-from)*k;music.volume=musicGain*MUSIC_VOL;if(k<1)fadeRaf=requestAnimationFrame(tick);else if(target===0)music.pause();})(t0);}
